workspace "CommerceOS - v3" "C4 model for CommerceOS, a Shopify-like platform for creating storefronts and selling products online. This version expands the backend with Catalog, Insights, Marketplace, and a TypeScript BFF." {
    ...
    model {
        ...
        group "CommerceOS" {
            ...
            admin = softwareSystem "Admin System" "Platform that Users use to manage storefronts." {
                ...
                bff = container "TypeScript BFF" "Browser-facing backend used by the Web App for Core API orchestration." "TypeScript"
                api = container "Core API" "Gateway API used by the Admin, Storefront, and BFF layers." "Go/Gin REST API"
                db = container "Core DB" "Stores users, stores, customer records, and platform configuration." "Postgres" {
                    tags "Database"
                }
                catalogApi = container "Catalog API" "Manages product catalog data." "TypeScript"
                catalogDb = container "Catalog DB" "Stores catalog data, categories, and product metadata." "Postgres" {
                    tags "Database"
                }
                ordersApi = container "Orders API" "Manages order lifecycle, order status, and fulfillment coordination." "TypeScript"
                ordersDb = container "Orders DB" "Stores order records, order events, and fulfillment state." "Postgres" {
                    tags "Database"
                }
                insightsApi = container "Insights API" "Aggregates reporting and analytics data." "TypeScript"
                marketplaceService = container "Marketplace Service" "Handles marketplace integrations and third-party marketplace publishing." "TypeScript"
        ...
        admin.web -> admin.bff "Uses" "HTTPS/JSON"
        admin.web -> admin.catalogApi "Manages catalog data" "HTTPS/JSON"
        admin.web -> admin.ordersApi "Manages orders" "HTTPS/JSON"
        admin.web -> admin.insightsApi "Views insights" "HTTPS/JSON"
        admin.web -> admin.marketplaceService "Manages marketplace integrations" "HTTPS/JSON"
        admin.web -> admin.publicApi "Builds and manages plugins" "HTTPS/JSON"
        admin.web -> admin.aiGateway "Uses AI-assisted workflows" "HTTPS/JSON"
        ...
        admin.bff -> admin.api "Orchestrates Core API access" "HTTPS/JSON"
        admin.api -> admin.db "Reads from and writes to" "SQL"
        admin.api -> admin.catalogApi "Delegates catalog operations" "HTTPS/JSON"
        admin.api -> admin.ordersApi "Delegates order operations" "HTTPS/JSON"
        admin.catalogApi -> admin.catalogDb "Reads from and writes to" "SQL"
        admin.ordersApi -> admin.ordersDb "Reads from and writes to" "SQL"
        admin.insightsApi -> admin.api "Aggregates commerce data" "HTTPS/JSON"
        admin.insightsApi -> admin.aiGateway "Requests AI-assisted analytics" "HTTPS/JSON"
        admin.marketplaceService -> admin.api "Synchronizes marketplace data" "HTTPS/JSON"
        admin.marketplaceService -> admin.publicApi "Publishes marketplace extensions" "HTTPS/JSON"
    ...
    views {
        ...
        container admin "AdminContainers" {
            ...
            include admin.api
            include admin.db
            include admin.catalogApi
            include admin.catalogDb
            include admin.ordersApi
            include admin.ordersDb
            include admin.insightsApi
            include admin.marketplaceService
